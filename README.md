...
└── health-check.sh (백그라운드)
    ...
    └── WARN/CRITICAL 시 runbook 기반 권장 조치 후속 전송
```
...
임계값 판단과 Webhook 알림 자체는 LLM을 호출하지 않는다. WARN/CRITICAL 알림이 실제 전송되면 백그라운드에서 권장 조치를 후속 메시지로 보낸다. runbook에서 검증된 deterministic mapping에 해당하는 알림은 LLM 없이 권장 조치를 보내고, 그 외 알림만 Gateway LLM fallback을 호출한다.
...
| 이상/복구 알림 | 상태 변경 시 | `health-check.sh` | runbook 매핑 없을 때만 | Discord Webhook 알림 + 권장 조치 후속 전송 |
...
WARN/CRITICAL 알림이 실제 전송되면 `health-check.sh`가 별도 백그라운드 작업으로 권장 조치를 후속 전송한다. runbook deterministic mapping은 `config/runbook-recommendations.json`의 `alert_key`와 `severities`로 판단한다. 현재 memory WARN/CRITICAL, SSH CRITICAL, Spring Boot health CRITICAL, 응답시간 WARN/CRITICAL, 에러율 WARN/CRITICAL, resource parse WARN에 적용된다. 이 파일은 `docs/runbook.md`의 절차를 알림 경로에서 안정적으로 쓰기 위한 구조화 source of truth이며, Markdown 문서를 런타임에 파싱하지 않는다. 그 외 알림은 Gateway `/v1/responses` LLM fallback을 호출한다. 중복 억제된 알림과 복구 알림에는 권장 조치를 붙이지 않는다.
...
runbook에 직접 대응 절차가 없으면 `runbook에 직접 절차 없음`을 먼저 밝히고, 프롬프트에 포함된 target-system 요약과 현재 snapshot/realtime metric만 근거로 `추론 기반 권장 조치`를 제안한다.
...
- 정기 이상 감지의 임계값 판단은 LLM 없이 동작한다. WARN/CRITICAL 알림 전송 성공 시 `config/runbook-recommendations.json`에 매칭되는 권장 조치는 LLM 없이 보내고, 그 외 알림만 LLM fallback을 사용한다. Full Report와 Daily Summary도 같은 JSON을 payload에 포함해 권장 조치 섹션을 먼저 매칭하고, 매칭이 없는 이슈만 target-system 요약과 snapshot 기반 fallback을 사용한다.
