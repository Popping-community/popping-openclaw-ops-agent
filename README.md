...
└── health-check.sh (백그라운드)
    ...
    └── WARN/CRITICAL 시 JSON 기반 권장 조치 후속 전송
```
...
임계값 판단과 Webhook 알림 자체는 LLM을 호출하지 않는다. WARN/CRITICAL 알림이 실제 전송 성공한 경우에만 백그라운드에서 권장 조치를 후속 메시지로 보낸다. `config/runbook-recommendations.json`에 `alert_key + severity` 매칭이 있는 알림은 LLM 없이 deterministic 권장 조치를 보내고, 매칭이 없는 알림만 Gateway readiness를 기다린 뒤 Gateway `/v1/responses` LLM fallback을 호출한다. 중복 억제된 알림과 복구 알림에는 권장 조치를 붙이지 않는다.
...
| 이상/복구 알림 | 상태 변경 시 | `health-check.sh` | 권장 조치 매핑 없을 때만 | Discord Webhook 알림 + 권장 조치 후속 전송 |
...
WARN/CRITICAL 알림이 실제 전송 성공한 경우에만 `health-check.sh`가 별도 백그라운드 작업으로 권장 조치를 후속 전송한다. deterministic recommendation은 `config/runbook-recommendations.json`의 `alert_key + severity`로 판단한다. 현재 memory WARN/CRITICAL, SSH CRITICAL, Spring Boot health CRITICAL, 응답시간 WARN/CRITICAL, 에러율 WARN/CRITICAL, resource parse WARN에 적용된다. 이 파일은 `docs/runbook.md`의 절차를 알림 경로에서 안정적으로 쓰기 위한 구조화 source of truth이며, Markdown 문서를 런타임에 파싱하지 않는다. 매칭이 없는 알림은 Gateway readiness를 기다린 뒤 Gateway `/v1/responses` LLM fallback을 호출한다. 중복 억제된 알림과 복구 알림에는 권장 조치를 붙이지 않는다.

자동 알림 fallback, Full Report, Daily Summary의 LLM 호출은 모두 OpenClaw Gateway `/v1/responses` 경로를 사용한다.
...
1. 자동 알림, Full Report, Daily Summary는 `config/runbook-recommendations.json`의 `alert_key + severity` 기반 recommendation을 먼저 매칭한다.
2. 사용자 직접 요청은 `docs/runbook.md`를 우선 기준으로 삼는다.
3. runbook에 직접 대응 절차가 없으면 `runbook에 직접 절차 없음`을 먼저 밝힌다.
4. 그 다음 프롬프트에 포함된 target-system 요약과 현재 snapshot/realtime metric만 근거로 `추론 기반 권장 조치`를 제안한다.
...
| `LLM_RECOMMENDATION_GATEWAY_READY_TIMEOUT_SECONDS` | 선택 | health-check 권장 조치 전 Gateway readiness 대기 시간. 기본값 `${GATEWAY_READY_TIMEOUT_SECONDS:-120}` |
| `RUNBOOK_RECOMMENDATIONS_FILE` | 선택 | 구조화 runbook recommendation JSON 경로. 기본값 `/root/.openclaw/config/runbook-recommendations.json` |
...
- 정기 이상 감지의 임계값 판단은 LLM 없이 동작한다. 권장 조치 기준은 위 `권장 조치 기준` 섹션을 따른다.
