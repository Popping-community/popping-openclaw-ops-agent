...
LLM은 사용자 요청, Full Report, Daily Summary, 그리고 `config/runbook-recommendations.json`에 없는 WARN/CRITICAL 알림의 fallback 권장 조치 생성에서 사용한다.
...
`config/runbook-recommendations.json`에 있는 알림은 LLM 없이 권장 조치를 보내고, 그 외 알림만 Gateway `/v1/responses` LLM fallback을 사용한다.
...
- `config/runbook-recommendations.json`에 있는 알림은 LLM 없이 권장 조치로 처리한다.
...
- WARN/CRITICAL 알림 시 runbook 기반 권장 조치를 자동 후속 전송해 운영자의 초기 대응 속도를 높인다.
