...
LLM은 사용자 요청, Full Report, Daily Summary, 그리고 WARN/CRITICAL 알림 후 권장 조치 생성에서 사용한다.
...
그래서 health-check가 WARN/CRITICAL 알림 전송에 성공한 직후, 백그라운드에서 Gateway `/v1/responses`를 호출해 LLM 기반 권장 조치를 후속 Discord 메시지로 자동 전송하도록 추가했다.

설계 원칙은 다음과 같다.

- 알림 판단과 전송은 여전히 LLM 없이 bash가 처리한다. LLM 권장 조치는 알림 이후 별도 백그라운드 작업이다.
- 권장 조치 생성이 실패해도 알림 자체는 정상 전송된다. LLM 호출 실패가 알림 파이프라인을 막지 않는다.
- LLM에게 보내는 입력은 알림 메시지와 현재 서버 snapshot이다. runbook에 절차가 있으면 우선하고, 없으면 target-system 문서 기반 추론 조치를 제안하도록 프롬프트에 명시했다.
...
- WARN/CRITICAL 알림 시 LLM 권장 조치를 자동 후속 전송해 운영자의 초기 대응 속도를 높인다.
...
- automatic alert recommendation (non-blocking)
- CI-gated deployment
