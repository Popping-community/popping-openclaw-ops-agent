...
LLM은 사용자 요청, Full Report, Daily Summary, 그리고 `config/runbook-recommendations.json`에 `alert_key + severity`가 매칭되지 않는 WARN/CRITICAL 알림의 fallback 권장 조치 생성에서 사용한다.

## 문제 2. 오래된 snapshot을 최신 상태처럼 믿을 수 있었다

서버 상태 보고는 빠르게 응답해야 한다.
그래서 health-check가 기본 10분마다 수집한 결과를 snapshot으로 저장하고, PoppingOps는 그 파일을 우선 읽는다.
...
- 그 다음 프롬프트에 포함된 target-system 요약과 현재 서버 메트릭만 근거로 `추론 기반 권장 조치`를 제안한다.

### WARN/CRITICAL 알림 시 자동 권장 조치

처음에는 권장 조치가 사용자가 Discord에서 질문했을 때만 제공되었다.
하지만 운영 알림이 왔을 때 사람이 직접 봇에 "어떻게 조치해?"를 물어야 하는 단계가 불필요했다.

그래서 health-check가 WARN/CRITICAL 알림 전송에 성공한 직후, 백그라운드에서 권장 조치를 후속 Discord 메시지로 자동 전송하도록 추가했다.
`config/runbook-recommendations.json`에 `alert_key + severity`가 매칭되는 알림은 LLM 없이 권장 조치를 보내고, 매칭이 없는 알림만 Gateway `/v1/responses` LLM fallback을 사용한다.

이 과정에서 fallback 권장 조치를 `openclaw:main` agent runtime 대신 direct provider model 호출로 분리하는 방안도 검토했다. 하지만 실제 실패는 provider 선택 문제가 아니라 health-check가 Gateway ready 전에 `/v1/responses`를 호출한 startup race였다. 이 문제는 Gateway readiness 대기로 직접 막을 수 있고, deterministic-first를 적용하면 memory WARN 같은 반복 알림은 LLM을 호출하지 않는다. direct provider를 추가하면 별도 인증, 모델명, payload, 에러 처리, rate limit 경로까지 새로 운영해야 해서 현재 이점이 작았다. 그래서 자동 fallback, Full Report, Daily Summary는 모두 Gateway `/v1/responses` 경로를 유지했다.

설계 원칙은 다음과 같다.

- 알림 판단과 전송은 여전히 LLM 없이 bash가 처리한다. 권장 조치는 알림 이후 별도 백그라운드 작업이다.
- `config/runbook-recommendations.json`에 `alert_key + severity`가 매칭되는 알림은 LLM 없이 권장 조치로 처리한다.
- Full Report와 Daily Summary도 같은 JSON을 payload에 포함해 권장 조치 섹션을 먼저 매칭한다.
- 권장 조치 생성이 실패해도 알림 자체는 정상 전송된다. LLM 호출 실패가 알림 파이프라인을 막지 않는다.
- LLM fallback에게 보내는 입력은 알림 메시지, target-system 요약, 현재 서버 snapshot이다. runbook에 직접 절차가 없음을 밝히고 bounded 추론 조치를 제안하도록 프롬프트에 명시했다.
- 복구 알림에는 권장 조치를 보내지 않는다. 복구는 조치가 필요한 상황이 아니기 때문이다.
...
- WARN/CRITICAL 알림 시 JSON 기반 권장 조치를 자동 후속 전송해 운영자의 초기 대응 속도를 높인다.
...
- automatic alert recommendation (deterministic-first, non-blocking)
- CI-gated deployment
