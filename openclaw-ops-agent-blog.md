...
- 재시작, 설정 변경, 배포, write operation은 사용자의 명시적 승인 없이는 실행하지 않는다.
- runbook에 없으면 `runbook에 직접 절차 없음`을 먼저 밝힌다.
- 그 다음 `추론 기반 권장 확인`으로 read-only 진단만 제안한다.
...
3. EC2 exporter 포트 8081/9100/9104 응답 여부 확인
4. 관련 metric의 rate_status_* 값이 init/missing/reset인지 확인

재시작이나 설정 변경은 사용자 승인 전에는 수행하지 않는다.
```
