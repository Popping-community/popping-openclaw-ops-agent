...
2. `debug1: connect to address <EC2_HOST> port 22: Connection timed out` 확인
...
- `workspace/SOUL.md`
  ...
  - runbook에 직접 절차가 없으면 `runbook에 직접 절차 없음`을 먼저 밝히고, 제공된 target-system 요약과 현재 snapshot/realtime metric 기반의 `추론 기반 권장 조치` 목록을 제안하도록 했다.
  - runbook 기반 권장 조치도 자동 실행이 아니라 안내이며, restart/config/deploy/write operation은 `운영자가 검토할 조치`로 분리하도록 했다.
- `workspace/skills/grafana-monitor/SKILL.md`
  - 서버 상태 보고의 권장 조치도 runbook-first로 작성하도록 동일한 매핑을 추가했다.
  - runbook에 없는 경우에는 `즉시 확인할 조치`와 `운영자가 검토할 조치`를 분리하고, 에이전트가 restart/delete/config/deploy/write operation을 직접 실행하지 않도록 했다.
...
## 36. Infra target 환경변수화

### 문제 상황

EC2 host, SSH port/user, actuator port, exporter port가 `entrypoint.sh`, `health-check.sh`, runbook, agent skill 문서에 직접 들어가 있었다.
인프라 host나 port가 바뀌면 코드와 문서를 수정하고 다시 배포해야 했다.

### 결정

운영 target 값을 필수 Railway Variables로 분리하고, 공개 저장소에 있던 실제 host/user/port 기본값을 제거했다.
...
`entrypoint.sh`는 이 값을 fail-fast 대상으로 검증하고, Gateway/agent 하위 프로세스도 같은 target 정보를 사용하도록 export한다.
`health-check.sh`는 자체 실행 시에도 값이 비어 있거나 port가 숫자가 아니면 즉시 실패한다.

### 효과

- EC2 IP, SSH port, actuator/exporter port 변경 시 Railway Variables만 바꾸면 된다.
- 공개 repository에 실제 인프라 host/user/port를 기본값으로 남기지 않는다.
- 자동 health-check와 Discord 실시간 서버 상태 skill이 같은 target 변수를 사용한다.
- runbook과 workspace tool 문서의 실행 명령도 환경변수 기반으로 정렬된다.

---

## 37. Target system 문서화와 구조 기반 fallback 조치

### 문제 상황

PoppingOps 자체 아키텍처는 `docs/architecture.md`에 정리되어 있었지만, 모니터링 대상인 Popping-community 서버의 실제 런타임 구조는 별도 문서로 분리되어 있지 않았다.

이 상태에서는 runbook에 없는 이상 상황이 발생했을 때 LLM이 현재 metric만 보고 조치를 추론하게 된다.
하지만 대상 서버는 단일 EC2에서 Spring Boot와 MySQL이 함께 실행되는 구조라 memory/load/disk/connection 문제를 해석할 때 실제 배포 구조가 중요하다.

### 확인

`C:\popping-community\popping-server`의 `docker-compose.yml`, `build.gradle`을 확인하고, Railway Variables의 SSH 접속 정보로 EC2에 read-only 접속해 실제 런타임 상태를 확인했다.

확인된 구조:

- Amazon Linux 2 단일 EC2
- 1 vCPU, 약 952 MiB memory, 30 GiB root disk
- Docker Compose 기반 실행
- Nginx: public `443 ssl`, Let's Encrypt TLS, Spring Boot `localhost:8080` reverse proxy
- `popping-community`: Spring Boot app, host `8080`, management `127.0.0.1:8081`
- `mysql`: MySQL 8
- `node-exporter`: `127.0.0.1:9100`
- `mysqld-exporter`: `127.0.0.1:9104`
- actuator health, actuator prometheus, node-exporter, mysqld-exporter 응답 확인

### 결정

`docs/target-system.md`를 추가해 PoppingOps가 감시하는 대상 서버 환경과 운영 해석 기준을 분리했다.

문서 책임은 다음처럼 나눈다.

- `docs/architecture.md`: PoppingOps 봇 자체 구조
- `docs/target-system.md`: 모니터링 대상 Popping-community 서버 구조
- `docs/runbook.md`: 장애 유형별 대응 절차

에이전트 지침도 수정했다.

- runbook에 대응 절차가 있으면 runbook 기반 조치를 우선한다.
- runbook에 직접 절차가 없으면 `runbook에 직접 절차 없음`을 먼저 밝힌다.
- 그 다음 제공된 target-system 요약과 현재 snapshot/realtime metric을 근거로 `추론 기반 권장 조치`를 제안한다.
- fallback 조치는 `즉시 확인할 조치`와 `운영자가 검토할 조치`로 분리한다.
- 에이전트는 재시작, 설정 변경, 배포, 삭제, write operation을 직접 실행하지 않는다.

### 효과

- runbook 밖 상황에서도 LLM의 일반 추론이 아니라 실제 대상 서버 구조에 묶인 조치 추천이 가능해진다.
- EC2 host resource, Spring Boot actuator, HikariCP, MySQL exporter 지표를 구조적으로 연결해 해석할 수 있다.
- PoppingOps 자체 아키텍처와 모니터링 대상 서버 아키텍처의 책임이 분리된다.
- 조치 추천은 유지하되 실행 경계는 `운영자가 검토할 조치`로 명확히 분리된다.

---

## 38. WARN/CRITICAL 알림 시 권장 조치 자동 전송

### 문제 상황

WARN/CRITICAL 알림이 Discord에 오면 운영자가 봇에 "어떻게 조치해?"를 직접 물어야 권장 조치를 받을 수 있었다.
알림과 권장 조치 사이에 사람의 추가 입력이 필요한 단계가 있었다.

### 결정

`health-check.sh`에서 WARN/CRITICAL 알림 전송이 성공한 직후, 백그라운드에서 권장 조치를 후속 Discord 메시지로 자동 전송하도록 추가했다.
이 시점의 구현은 "알림 전송 성공 후 Gateway `/v1/responses`로 LLM 권장 조치를 요청하고, 응답을 Discord 후속 메시지로 보낸다"는 구조였다.

설계 원칙:

- 알림 판단과 전송은 여전히 LLM 없이 bash가 처리한다. 권장 조치는 알림 이후 별도 백그라운드(`&`) 작업이다.
- 권장 조치는 `send_alert`가 Discord 2xx 응답을 받은 WARN/CRITICAL 알림에만 붙인다. 중복 억제된 알림과 복구 알림은 제외한다.
- 권장 조치 요청은 Gateway `/v1/responses`를 호출한다.
- 권장 조치 생성이 실패해도 1차 알림 자체에는 영향을 주지 않는다. LLM 호출 실패가 알림 파이프라인을 막지 않는다.
- Gateway token은 `OPENCLAW_GATEWAY_TOKEN`, `GATEWAY_TOKEN`, `/root/.openclaw/openclaw.json` 순서로 fallback한다.
- Gateway `/v1/responses`가 일시 실패할 수 있으므로 권장 조치 호출은 기본 6회, 10초 간격으로 재시도한다.
- Discord 2000자 제한을 피하기 위해 권장 조치 후속 메시지는 1900자 단위로 나눠 전송한다.
- Gateway가 `Agent couldn't generate a response` 같은 agent 실패 문구를 HTTP 200 본문으로 돌려주면 후속 Discord 메시지를 보내지 않고 로그만 남긴다.
- 복구 알림에는 권장 조치를 보내지 않는다.

### 효과

- 알림 직후 운영자가 추가 질문 없이 권장 조치를 바로 확인할 수 있다.
- 알림 파이프라인의 안정성은 유지된다 — LLM 호출 실패가 알림을 막지 않는다.
- 정상 상태 반복 체크에서는 여전히 LLM을 호출하지 않는다.
- 다만 배포 직후 health-check가 Gateway ready보다 먼저 WARN을 감지하면 첫 권장 조치 호출이 `http=000`으로 실패할 수 있다는 한계가 남았다.

---

## 39. 권장 조치 실패 대응 — Gateway readiness와 deterministic-first

### 문제 상황

38번 구현이 배포된 뒤, Railway startup 직후 health-check가 Gateway보다 먼저 실행되면서 메모리 WARN 알림이 먼저 발생했다.
로그에서는 `LLM recommendation request failed (http=000, attempt=1/6)`이 먼저 찍히고, 그 뒤에야 Gateway HTTP server가 ready 상태가 되었다.

즉 1차 Discord 알림은 정상 전송됐지만, 후속 권장 조치는 Gateway 준비 타이밍과 LLM runtime 실패에 영향을 받았다.

### 결정

후속 대응으로 세 가지 방안을 검토했다.

1. health-check는 Gateway보다 먼저 띄우되, LLM 권장 조치는 Gateway ready 이후에만 시작한다.
2. 권장 조치 fallback을 `openclaw:main` agent runtime이 아닌 direct provider model 호출로 분리한다.
3. LLM 실패 시 runbook 기반 deterministic fallback을 보낸다.

실제로 적용한 것은 1번과 3번을 결합한 구조다.

- `health-check.sh` 내부에 Gateway readiness 대기를 추가했다.
- 권장 조치는 LLM보다 runbook deterministic mapping을 먼저 확인하도록 바꿨다.
- memory WARN/CRITICAL, SSH CRITICAL, Spring Boot Health CRITICAL, avg_response WARN/CRITICAL, error_rate WARN/CRITICAL, resource_parse WARN은 LLM 없이 runbook 기반 권장 조치로 처리한다.
- 매핑이 없는 알림만 Gateway `/v1/responses` LLM fallback을 호출한다.
- direct provider model 호출은 구현하지 않았다. 실제 실패 원인은 provider 선택 문제가 아니라 health-check가 Gateway ready 전에 `/v1/responses`를 호출한 startup race였고, 이 문제는 Gateway readiness 대기로 직접 완화할 수 있었다.
- deterministic-first 적용 후에는 memory WARN/CRITICAL, SSH CRITICAL, app health CRITICAL, 응답시간/에러율 WARN/CRITICAL, resource parse WARN 같은 반복 알림이 LLM 없이 처리된다. 따라서 direct provider를 추가해 줄일 수 있는 호출 범위가 작아졌다.
- direct provider 경로는 별도 인증, 모델명, payload, 에러 처리, rate limit 정책을 새로 운영해야 한다. Full Report와 Daily Summary는 여전히 Gateway의 agent runtime 문맥을 쓰는 편이 자연스럽기 때문에, 자동 알림 fallback만 별도 provider 경로로 분리하면 운영 경로가 둘로 갈라진다.
- 그래서 현재 단계에서는 direct provider를 보류하고, fallback 실패가 반복되거나 Gateway direct provider 호출 지원이 명확해질 때 2단계 fallback으로 재검토한다.
- 따라서 현재 LLM fallback은 여전히 Gateway `/v1/responses`를 통해 `openclaw:main` model 경로를 사용한다.

### 효과

- Gateway startup race 때문에 첫 권장 조치가 바로 실패하는 문제를 줄였다.
- 반복적이고 runbook에 매핑된 알림은 Gateway/LLM 상태와 무관하게 최소 조치를 받을 수 있다.
- LLM fallback은 runbook 밖 이슈에만 사용되어 비용과 실패 영향 범위가 줄었다.
- direct provider 호출은 후속 검토 과제로 남겼다.

---

## 40. 구조화 runbook recommendation — 자동 알림과 리포트의 공통 조치 기준

### 문제 상황

39번에서 자동 알림 후속 권장 조치를 deterministic-first로 바꿨지만, 기준이 코드와 문서에 흩어질 수 있었다.
또한 Full Report와 Daily Summary는 LLM 보고서 작성 경로라서 "runbook 우선" 지시만으로는 권장 조치의 기준이 자동 알림과 완전히 같다고 보장하기 어려웠다.
앞선 검토에서 direct provider 호출은 채택하지 않았기 때문에, Gateway `/v1/responses` fallback을 유지하면서도 LLM이 맡는 범위를 더 줄이는 쪽으로 설계를 정리했다.

### 결정

`config/runbook-recommendations.json`을 추가해 실행 가능한 runbook recommendation을 구조화했다.
`docs/runbook.md`는 사람이 읽는 운영 절차 문서로 유지하고, 자동 경로에서 매칭 가능한 권장 조치의 source of truth는 JSON으로 분리했다.

설계 원칙:

- JSON은 `alert_key`, `severities`, `runbook_section`, `basis_metrics`, `immediate_checks`, `operator_review_actions`를 가진다.
- `health-check.sh`는 Markdown runbook을 파싱하지 않고 JSON의 `alert_key + severities`로 deterministic recommendation을 매칭한다.
- 매칭되면 LLM 없이 Discord 후속 권장 조치를 보낸다.
- 매칭이 없을 때만 Gateway `/v1/responses` LLM fallback을 호출한다.
- `target_system_summary`도 JSON에 포함해 자동 fallback과 리포트가 같은 서버 요약을 사용한다.
- Full Report와 Daily Summary는 같은 JSON을 payload에 포함해 권장 조치 섹션을 먼저 매칭하고, 매칭 없는 이슈에만 target-system 요약과 snapshot 기반 fallback을 사용한다.
- `heartbeat-context.sh`는 현재 snapshot 기준으로 `runbook_active_recommendations_json`을 생성해 리포트가 현재 이슈에 맞는 runbook 조치를 우선 사용할 수 있게 했다.

### 효과

- 자동 알림, Full Report, Daily Summary의 권장 조치 기준이 하나의 구조화 파일로 통일된다.
- Markdown 문서 포맷 변경이 알림 경로를 깨뜨리지 않는다.
- 반복 장애 조치는 토큰 비용 없이 안정적으로 안내되고, runbook 밖 이슈만 bounded LLM fallback으로 처리된다.
- runbook 절차 변경 시 사람이 읽는 문서와 실행용 JSON을 함께 갱신해야 한다는 운영 경계가 명확해졌다.
...
| 36 | EC2 host/port/exporter port가 코드와 skill에 하드코딩됨 | 인프라 target 값을 필수 Railway Variables로 분리 | 인프라 식별 정보 노출과 코드 수정/재배포 필요성 감소 |
| 37 | runbook 밖 상황에서 대상 서버 구조 근거가 부족함 | `docs/target-system.md` 추가 + fallback 조치 기준 연결 | 실제 EC2/Docker/Spring/MySQL 구조와 현재 메트릭 기반 조치 추천 |
| 38 | WARN/CRITICAL 알림 뒤 권장 조치를 받으려면 운영자가 다시 질문해야 함 | 알림 전송 성공 후 Gateway `/v1/responses`로 LLM 권장 조치를 비동기 후속 전송 | 초기 대응 속도 향상, 1차 알림과 권장 조치 실패 영향 분리 |
| 39 | 배포 직후 Gateway ready 전에 LLM 권장 조치가 호출되어 실패함 | Gateway readiness 대기 후 runbook 기반 권장 조치를 먼저 전송하고, 매핑 없는 알림만 Gateway `/v1/responses` fallback 사용. direct provider 호출은 원인 대비 이점이 작고 운영 경로가 늘어 보류 | startup race 완화, LLM 비용/실패 영향 축소 |
| 40 | 자동 알림과 리포트의 권장 조치 기준이 흩어질 수 있음 | `config/runbook-recommendations.json`으로 실행 가능한 runbook recommendation을 구조화하고 health-check/Full Report/Daily Summary가 공유 | 조치 기준 일관성 강화, Markdown 파싱 의존 제거, LLM fallback 범위 축소 |
