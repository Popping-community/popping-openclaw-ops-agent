...
- 프롬프트에 runbook(`docs/runbook.md`) 우선, 없으면 `docs/target-system.md` 기반 추론 조치를 제안하도록 명시했다.
...
| 38 | WARN/CRITICAL 알림 뒤 권장 조치를 받으려면 운영자가 다시 질문해야 함 | 알림 전송 성공 후 Gateway LLM 권장 조치를 비동기로 후속 전송 | 초기 대응 속도 향상, 알림 파이프라인과 LLM 실패 영향 분리 |
