...
- `workspace/SOUL.md`
  ...
  - runbook에 직접 절차가 없으면 `runbook에 직접 절차 없음`을 먼저 밝히고, target-system 문서의 실제 서버 환경/아키텍처와 현재 snapshot/realtime metric 기반의 `추론 기반 권장 조치` 목록을 제안하도록 했다.
...
- 그 다음 `docs/target-system.md`의 실제 서버 환경/아키텍처와 현재 snapshot/realtime metric을 근거로 `추론 기반 권장 조치`를 제안한다.
...
## 38. WARN/CRITICAL 알림 시 LLM 권장 조치 자동 전송
...
`health-check.sh`에서 WARN/CRITICAL 알림 전송이 성공한 직후, 백그라운드에서 Gateway `/v1/responses`를 호출해 LLM 기반 권장 조치를 후속 Discord 메시지로 자동 전송하도록 추가했다.

설계 원칙:

- 알림 판단과 전송은 여전히 LLM 없이 bash가 처리한다. LLM 권장 조치는 알림 이후 별도 백그라운드(`&`) 작업이다.
- LLM 권장 조치는 `send_alert`가 Discord 2xx 응답을 받은 WARN/CRITICAL 알림에만 붙인다. 중복 억제된 알림과 복구 알림은 제외한다.
- 권장 조치 생성이 실패해도 1차 알림 자체에는 영향을 주지 않는다. LLM 호출 실패가 알림 파이프라인을 막지 않는다.
- LLM에게 보내는 입력은 알림 메시지와 현재 서버 snapshot(`status-current.env`)이다.
- 프롬프트는 도구/명령/파일 읽기 없이 제공된 알림 메시지와 snapshot만 사용하도록 제한한다.
- 메모리, SSH, Spring Boot Health, Webhook, snapshot stale, 응답시간, 에러율 알림은 runbook 대응 절차가 있는 것으로 보고 runbook-first로 안내한다.
- 그 외 알림은 `runbook에 직접 절차 없음`을 밝힌 뒤 현재 snapshot 기반 추론 조치를 제안한다.
- 복구 알림에는 권장 조치를 보내지 않는다.
- Gateway token은 `OPENCLAW_GATEWAY_TOKEN`, `GATEWAY_TOKEN`, `/root/.openclaw/openclaw.json` 순서로 fallback한다.
- Gateway가 아직 뜨는 중일 수 있으므로 권장 조치 호출은 기본 6회, 10초 간격으로 재시도한다.
...
| 38 | WARN/CRITICAL 알림 뒤 권장 조치를 받으려면 운영자가 다시 질문해야 함 | 알림 전송 성공 후 Gateway LLM 권장 조치를 비동기로 후속 전송하고 agent 실패 문구는 억제 | 초기 대응 속도 향상, 알림 파이프라인과 LLM 실패 영향 분리 |
