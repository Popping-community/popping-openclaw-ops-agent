...
| 36 | EC2 host/port/exporter port가 코드와 skill에 하드코딩됨 | 인프라 target 값을 Railway Variables로 override 가능하게 변경 | 인프라 변경 시 코드 수정/재배포 필요성 감소 |
...
운영 target 값을 환경변수로 분리하고 기존 운영값은 default로 유지했다.
...
`entrypoint.sh`는 이 값을 export해서 Gateway/agent 하위 프로세스도 같은 target 정보를 사용하게 한다.
`health-check.sh`는 자체 실행 시에도 default를 적용하고 port 값은 숫자인지 검증한다.
