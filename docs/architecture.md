...
WARN/CRITICAL 알림이 실제 전송 성공한 경우에만 백그라운드에서 권장 조치를 후속 Discord 메시지로 보낸다. `config/runbook-recommendations.json`에 `alert_key + severity`가 매칭되는 알림은 LLM 없이 권장 조치를 사용하고, 매칭이 없는 알림만 Gateway readiness를 기다린 뒤 Gateway `/v1/responses` LLM fallback을 호출한다. 이 후속 단계는 알림 판단에 영향을 주지 않으며, 실패해도 1차 알림 자체에는 영향을 주지 않는다.
